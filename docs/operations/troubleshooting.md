...
| A staged doc shows a **Conflict** badge and Commit is disabled / commit error says *"Unresolved conflict"* | The doc's identity (`doc_id` = type+project+equipment+title+error_codes) already exists in the KB, so committing would overwrite it | Open **Compare & resolve** and pick keep / overwrite / merge (`PATCH …/documents/{idx}/resolve`). To revert to the old silent-overwrite behaviour set `KB_INGEST__COLLISION_DETECTION_ENABLED=false` |
